...
class MakeResource {
  ...
  public async run() {
    ...
    // 2. Create API Resource (always generated by default)
    this.createApiResource(pascalName, lowerName);
    ...
    console.log(`  ${c.dim}3. Export resource in ${c.cyan}app/http/resources/index.ts${c.reset}`);
    if (withModel) {
      console.log(`  ${c.dim}4. Export model in ${c.cyan}app/models/index.ts${c.reset}`);
      console.log(`  ${c.dim}5. Create migration: ${c.cyan}node nara make:migration create_${pluralName}_table${c.reset}`);
    } else {
      console.log(`  ${c.dim}4. Create migration: ${c.cyan}node nara make:migration create_${pluralName}_table${c.reset}`);
      console.log(`  ${c.dim}5. Create model: ${c.cyan}node nara make:model ${pascalName}${c.reset}`);
    }
    if (withPages) {
      console.log(`  ${c.dim}6. Customize Svelte pages in ${c.cyan}resources/js/Pages/${pascalName}/${c.reset}`);
    }
    console.log();
  }
  ...
  private createApiResource(pascalName: string, lowerName: string): boolean {
    const resourceClassName = pascalName + "Resource";
    const filename = resourceClassName + ".ts";
    const filepath = "./app/http/resources/" + filename;

    // Ensure resources directory exists
    if (!fs.existsSync("./app/http/resources")) {
      fs.mkdirSync("./app/http/resources", { recursive: true });
    }

    if (fs.existsSync(filepath)) {
      printInfo(`API Resource already exists: ${c.dim}${filepath}${c.reset}`);
      return false;
    }

    fs.writeFileSync(filepath, this.getApiResourceTemplate(resourceClassName, pascalName, lowerName));
    printSuccess(`Created: ${c.cyan}app/http/resources/${filename}${c.reset}`);

    // Show export hint
    console.log(`${c.dim}  Add to app/http/resources/index.ts:${c.reset}`);
    console.log(`${c.dim}    export { ${resourceClassName}, ${lowerName}Collection } from './${resourceClassName}';${c.reset}`);
  ...
  private generateRouteSnippet(pascalName: string, lowerName: string, pluralName: string, apiOnly: boolean) {
    ...
    if (apiOnly) {
      ...
      console.log(`import { ${resourceClassName} } from "@http/resources/${resourceClassName}";`);
      console.log();
...
import { ${pascalName}Resource, ${lowerName}Collection } from "@http/resources/${pascalName}Resource";
...
class ${className} extends BaseController {
  ...
  public async index(request: NaraRequest, response: NaraResponse) {
    ...
    return jsonPaginated(response, SUCCESS_MESSAGES.DATA_RETRIEVED, ${lowerName}Collection(result.data, result.meta).toArray(), result.meta);
  }
  ...
  public async show(request: NaraRequest, response: NaraResponse) {
    ...
    return jsonSuccess(response, SUCCESS_MESSAGES.DATA_RETRIEVED, new ${pascalName}Resource(record).toJson());
  }
  ...
  public async store(request: NaraRequest, response: NaraResponse) {
    ...
    try {
      ...
      return jsonCreated(response, SUCCESS_MESSAGES.DATA_CREATED, new ${pascalName}Resource(record).toJson());
    } catch (error: any) {
  ...
  public async update(request: NaraRequest, response: NaraResponse) {
    ...
    try {
      ...
      return jsonSuccess(response, SUCCESS_MESSAGES.DATA_UPDATED, new ${pascalName}Resource(record).toJson());
    } catch (error: any) {
...
`;
  ...
  private getApiResourceTemplate(resourceClassName: string, className: string, lowerName: string): string {
    return `/**
 * ${resourceClassName}
 *
 * Transforms ${className}Record to JSON API response format.
 * Demonstrates conditional field inclusion and relationship embedding.
 *
 * @example
 * // Basic usage
 * return jsonSuccess(response, '${className} found', new ${resourceClassName}(${lowerName}).toJson());
 *
 * @example
 * // With conditional fields
 * const resource = new ${resourceClassName}(${lowerName})
 *   .when(isAdmin, { email: ${lowerName}.email });
 * return jsonSuccess(response, '${className} found', resource.toJson());
 *
 * @example
 * // Collection
 * const ${lowerName}s = await ${className}.all();
 * const collection = ResourceCollection.of(${resourceClassName}, ${lowerName}s);
 * return jsonSuccess(response, '${className}s found', collection.toArray());
 *
 * @example
 * // With pagination
 * const result = await paginate<${className}Record>(query, { page, limit });
 * const collection = ResourceCollection.of(${resourceClassName}, result.data, result.meta);
 * return jsonPaginated(response, '${className}s found', collection.toArray(), result.meta);
 */

import { Resource } from '../Resource';
import { ResourceCollection } from '../ResourceCollection';
import type { ${className}Record } from '@models';
import type { PaginatedMeta } from '@services/Paginator';

/**
 * Resource class for transforming ${className}Record to JSON
 */
export class ${resourceClassName} extends Resource<${className}Record> {
  /**
   * Transform the ${lowerName} resource to an array
   *
   * @returns Object containing the transformed ${lowerName} data
   */
  toArray(): Record<string, unknown> {
    const ${lowerName} = this.resource;

    return {
      id: ${lowerName}.id,
      // TODO: Add your fields here (avoid exposing sensitive data like passwords)
      // name: ${lowerName}.name,
      created_at: ${lowerName}.created_at,
      updated_at: ${lowerName}.updated_at,
    };
  }
   ...
   *
   * @param meta - Metadata object to include
   * @returns this for method chaining
   *
   * @example
   * const resource = new ${resourceClassName}(${lowerName}).withMeta({ timestamp: Date.now() });
   */
  withMeta(meta: Record<string, unknown>): this {
    return this.with('meta', meta);
  }

  // TODO: Add more helper methods for conditional fields and relationships
}

/**
 * Create a collection of ${lowerName} resources
 *
 * @param ${lowerName}s - Array of ${lowerName} records
 * @param meta - Optional pagination metadata
 * @returns ResourceCollection instance
 *
 * @example
 * const ${lowerName}s = await ${className}.all();
 * return jsonSuccess(response, '${className}s found', ${lowerName}Collection(${lowerName}s).toArray());
 *
 * @example
 * // With pagination
 * const result = await paginate<${className}Record>(query, { page, limit });
 * const collection = ${lowerName}Collection(result.data, result.meta);
 * return jsonPaginated(response, '${className}s found', collection.toArray(), result.meta);
 */
export function ${lowerName}Collection(${lowerName}s: ${className}Record[], meta?: PaginatedMeta): ResourceCollection<${className}Record> {
  return ResourceCollection.of(${resourceClassName}, ${lowerName}s, meta);
}

export default ${resourceClassName};
`;
  }
