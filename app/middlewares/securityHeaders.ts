...
export function securityHeaders(options: SecurityHeadersOptions = {}): NaraMiddleware {
  ...
  return (_req: NaraRequest, res: NaraResponse, next: () => void) => {
    ...
    // Set to '0' to disable browser's XSS filter (modern approach)
    ...
    if (options.coep) {
      res.setHeader('Cross-Origin-Embedder-Policy', options.coep);
    }
    if (options.coop) {
      res.setHeader('Cross-Origin-Opener-Policy', options.coop);
    }
    if (options.corp) {
      res.setHeader('Cross-Origin-Resource-Policy', options.corp);
    }
