<script lang="ts">
  ...
  import { ArrowRight, ArrowUpRight, Eye, EyeOff, Sparkles } from '@lucide/svelte'
...
<div class="min-h-[100dvh] bg-background text-foreground font-body antialiased selection:bg-primary/20 selection:text-primary grid grid-cols-1 lg:grid-cols-2">
  ...
  <aside class="relative hidden lg:block overflow-hidden bg-muted">
    <img
      src="https://picsum.photos/seed/nara-begin-quiet/1400/1800"
    ...
    <div class="relative z-10 h-full flex flex-col justify-between p-12 xl:p-16">
      ...
      <div class="max-w-[28ch]">
        <p class="font-heading text-[11px] uppercase tracking-[0.25em] text-white/60 mb-5">No. 03 &nbsp;/&nbsp; The beginning</p>
        <p class="font-heading font-medium tracking-[-0.02em] leading-[1.1] text-[clamp(1.75rem,3vw,2.5rem)] text-white">
          Start with one prompt. Ship before lunch.
        </p>
      </div>
    </div>
  </aside>
  ...
  <main class="relative flex flex-col min-h-[100dvh]">
    ...
    <div class="relative z-10 flex-1 flex items-center px-6 sm:px-10 lg:px-12 py-10">
      <div class="w-full max-w-[420px] mx-auto" in:fly={{ y: 20, duration: 800, delay: 100 }}>
        ...
        >
          ...
          <span class="font-body text-sm text-foreground">Sign up with Google</span>
        </a>
        ...
        <!-- form -->
        <form class="flex flex-col gap-4" onsubmit={(e) => { e.preventDefault(); submitForm() }}>
          <div class="flex flex-col gap-2">
            <Label for="name" class="text-xs uppercase tracking-widest font-heading text-muted-foreground">Full name</Label>
            <Input bind:value={form.name} required type="text" name="name" id="name" placeholder="Your name" class="rounded-sm h-11" />
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="flex flex-col gap-2">
              <Label for="confirm-password" class="text-xs uppercase tracking-widest font-heading text-muted-foreground">Confirm</Label>
              <div class="relative">
                <Input bind:value={form.password_confirmation} required type={showConfirm ? 'text' : 'password'} name="confirm-password" id="confirm-password" placeholder="••••••••" class="pr-10 rounded-sm h-11" />
                <button type="button" onclick={() => (showConfirm = !showConfirm)} class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors cursor-pointer" aria-label={showConfirm ? 'Hide password' : 'Show password'}>
                  {#if showConfirm}
                    <EyeOff class="w-4 h-4" />
                  {:else}
                    <Eye class="w-4 h-4" />
          ...
          <button
            ...
            class="self-end inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors cursor-pointer group"
          >
            <Sparkles class="w-3.5 h-3.5 transition-transform group-hover:scale-110" />
            Generate a secure one
          </button>

          <Button type="submit" disabled={isLoading} size="lg" class="w-full rounded-sm h-12 normal-case tracking-normal font-heading font-medium text-sm mt-1">
            {#if isLoading}
              Creating account...
            {:else}
              Begin
          ...
          <p class="text-center text-muted-foreground text-sm pt-2">
            ...
            <a href="/login" use:inertia class="text-foreground hover:text-primary transition-colors underline underline-offset-4 decoration-border">Sign in</a>
          </p>
        </form>
      </div>
    </div>
