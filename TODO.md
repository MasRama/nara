...
> Verified 2026-09-04 plus adversarial follow-up: Content-Type-independent API body bound (text/plain and alternate media types rejected with 413, small bodies pass, multipart exempt), multipart early bound before parseBody (5 MB file + 256 KiB framing, Feature check authoritative), explicit TRUST_PROXY/TRUST_PROXY_HOPS trusted-suffix IP model (default socket-authoritative, leftmost never honored, malformed falls back), logout in auth limiter with CSRF preserved, 10_000-key oldest-first ceilings on both limiter stores, Auth-owned role validation over shared generic primitives, real production header smoke on / and /api/auth/me (CSP, nosniff, HSTS), dev Vite HTML explicitly out of Hono header scope with production authoritative. `src/shared/security/tests/security.test.ts` (43 tests) plus production-serving smoke (7 tests); `npm run check` (38 files/169 tests), `npm run build`, `npm run architecture:doctor`, `npm run test:production-serving` passed.
