...
class DbFresh {
  ...
  public async run() {
    ...
    // First rollback all
    const rollback = spawn('npx', ['tsx', 'node_modules/.bin/knex', 'migrate:rollback', '--all', '--env', env], {
      stdio: 'inherit',
      shell: true,
    });
    ...
    rollback.on('close', (rollbackCode) => {
      ...
      // Then migrate
      const migrate = spawn('npx', ['tsx', 'node_modules/.bin/knex', 'migrate:latest', '--env', env], {
        stdio: 'inherit',
        shell: true,
      });
      ...
      migrate.on('close', (migrateCode) => {
        ...
        if (seedFlag) {
          // Run seeders
          const seed = spawn('npx', ['tsx', 'node_modules/.bin/knex', 'seed:run', '--env', env], {
            stdio: 'inherit',
            shell: true,
          });
