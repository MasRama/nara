...
Nara delivers exceptional performance compared to pure Node.js, thanks to HyperExpress's optimized HTTP server implementation.
...
| Framework | Requests/sec | Avg Latency | Total Requests (30s) | vs Nara |
|-----------|--------------|-------------|----------------------|---------|
| **Nara (HyperExpress)** | **258,611** | **1.52ms** | **7,759,334** | **Baseline** |
...
**Key Insights:**
- **vs Pure Node.js**: Nara achieves **2.08x more requests per second** with **58% lower latency**
- **vs Express.js**: Nara is **11.45x faster** than the most popular Node.js framework
- **vs Pure PHP**: Nara handles **328x more requests per second** (258,611 vs 787 req/s)
- **vs Laravel**: Nara is **3,232x faster** (258,611 vs 80 req/s)
...
```bash
# Start Nara benchmark test server (port 3006)
node benchmark/nara-test.js
...
Nara uses BetterSQLite3 with WAL (Write-Ahead Logging) journal mode for exceptional database performance. The framework provides both native better-sqlite3 access and Knex.js query builder.
...
Nara provides both native better-sqlite3 (`app/services/SQLite.ts`) and Knex.js (`app/services/DB.ts`) for flexibility. Tested on Mac M4.
...
Follow these steps to deploy your Nara application to a production server:
...
Squirrelly is a lightweight and fast template engine used for server-side HTML rendering. Nara provides a `View` service that automatically loads all files from `resources/views` (development) or `dist/views` (production), supports partials, and adjusts asset paths during development.
...
class Controller {
  public async hello(request: Request, response: Response) {
    const html = view("hello.html", { title: "Hello Page", name: "Nara" });
...
Priority approach in Nara: generate a pre-signed URL on the server, then perform direct upload from browser/client to that URL using `PUT` method. This reduces server load while maintaining security.

### Prerequisites
- Set up Wasabi/S3 credentials in `.env` (see `.env.example`):
  - `WASABI_ACCESS_KEY`, `WASABI_SECRET_KEY`
  - `WASABI_BUCKET` (default: `nara-dev`)
...
You can build complete Nara applications using AI coding assistants without writing any code manually. This approach leverages AI editors to handle all implementation details while you focus on requirements and architecture.
...
```markdown
# TODOLIST - Nara Task Management Application
...
- **Use the framework**: Leverage Nara's conventions (controllers, Inertia pages, migrations)
...
- **[Nara](https://github.com/MasRama/nara)** - Modern full-stack TypeScript framework
...
If you find Nara useful, consider supporting its development:
...
- Open an issue on [GitHub](https://github.com/MasRama/nara/issues)
