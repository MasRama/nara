...
import { Toast, password_generator } from '../../components/helper';
...
function submit() {
  ...
  // Ensure phone is string as in Svelte logic
  const submitData = {
    ...form.data(),
    phone: form.phone.toString()
  };

  form.transform(() => submitData).post('/register');
}
</script>
...
<template>
  <div class="min-h-screen bg-white dark:bg-black text-slate-900 dark:text-slate-100 flex overflow-hidden">
    ...
    <div class="w-full lg:w-1/2 flex flex-col justify-between p-8 lg:p-12 z-10 relative overflow-y-auto order-1 lg:order-2">
        <div>
            <Link href="/" class="text-2xl font-bold tracking-tighter flex items-center gap-2 justify-end lg:justify-start">
                <NaraIcon />
                <span>NARA.</span>
            </Link>
        </div>
        ...
        <div class="max-w-md w-full mx-auto mt-8 lg:mt-0">
            ...
            <form class="space-y-4" @submit.prevent="submit">
                <div class="space-y-1">
                    ...
                    <input v-model="form.name" required type="text" name="name" id="name"
                    ...
                    <span v-if="form.errors.name" class="text-xs text-red-500 ml-1">{{ form.errors.name }}</span>
                </div>
                ...
                <div class="flex justify-end">
                  <button type="button" @click="generatePassword" class="text-xs font-mono text-info-400 hover:text-info-300 transition-colors flex items-center gap-1">
                ...
                <button type="submit" :disabled="form.processing"
                    ...
                    <span class="relative z-10">{{ form.processing ? 'CREATING...' : 'CREATE ACCOUNT' }}</span>
                ...
                <p class="text-center text-slate-500 dark:text-slate-400 mt-6">
                    Already have an account? <Link href="/login" class="font-bold text-info-500 dark:text-info-400 hover:underline">Login</Link>
                </p>
            </form>
        </div>
