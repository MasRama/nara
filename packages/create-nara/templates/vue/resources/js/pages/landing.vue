...
const page = usePage();
const currentUser = computed(() => page.props.user as User | undefined);

const scrolled = computed(() => false); // Simple reactive state for scroll if needed
</script>
...
<template>
  <div class="min-h-screen bg-surface-light dark:bg-surface-dark text-slate-900 dark:text-slate-100 transition-colors duration-500 overflow-x-hidden selection:bg-primary-400 selection:text-black">
    ...
    <!-- Floating Navigation -->
    <nav
      class="fixed top-0 left-0 right-0 z-50 px-6 py-6 flex justify-between items-start transition-all duration-500"
      :class="scrolled ? 'bg-white/80 dark:bg-black/80 backdrop-blur-md border-b border-slate-200 dark:border-white/10 py-4' : 'mix-blend-difference text-white'"
    >
      <div class="flex flex-col relative z-10">
        <Link href="/" class="text-xl font-bold tracking-tighter hover:opacity-70 transition-opacity">NARA.</Link>
      </div>
      ...
      <div class="flex items-center gap-6 text-sm font-medium relative z-10">
        ...
        <Link v-if="currentUser" href="/dashboard" class="hover:underline decoration-1 underline-offset-4">Dashboard</Link>
        <Link v-else href="/login" class="hover:underline decoration-1 underline-offset-4">Login</Link>
      ...
      <div class="px-6 sm:px-12 lg:px-24">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div class="group relative h-64 bg-surface-card-light dark:bg-surface-card-dark border border-slate-200 dark:border-white/5 hover:border-warning-500/50 dark:hover:border-warning-500/30 p-6 flex flex-col justify-between transition-all duration-500 overflow-hidden rounded-2xl">
             ...
             <div class="relative z-10">
               <h4 class="text-xl font-bold mb-2">Vue 3</h4>
               <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                 The canvas. Pure reactivity, elegant Composition API.
               </p>
       ...
       <div class="relative z-10 pt-32 pb-20 px-6 sm:px-12 lg:px-24 flex flex-col items-center text-center">
          ...
          <div class="mt-16 flex flex-col items-center gap-10">
             ...
             <div class="flex flex-col sm:flex-row items-center gap-6">
                <Link href="/docs" class="group relative px-10 py-5 bg-slate-900 dark:bg-white text-white dark:text-black font-bold text-lg tracking-tight overflow-hidden rounded-full hover:scale-105 transition-transform duration-300">
