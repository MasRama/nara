 ...
 */
let DB = DBInstance((config as any)[process.env.DB_CONNECTION as string]) as DBType;
...
function applySQLitePragmas(instance: Knex) {
  const client = ((config as any)[process.env.DB_CONNECTION as string] || {}).client;
...
DB.connection = (stage: string) => {
   const instance = DBInstance((config as any)[stage]) as DBType;
