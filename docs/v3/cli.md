...
Migrations are forward-only and checksummed in `_nara_migrations`. Applied migration files are immutable; create a new corrective migration instead of editing history. Startup applies pending migrations before Hono listens. See [`database-lifecycle.md`](./database-lifecycle.md) for the SQLite layout, WAL settings, compatibility behavior, and local-disk deployment boundary. The minimal project generated by `nara new` has no database Feature, so these scripts are supplied by the full Nara application rather than copied into a health-only starter.
